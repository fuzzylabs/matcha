...
def provision(
    ...
    resources = STACK_RESOURCE_MSG_DICT[stack]

    if is_user_approved(verb="provision", resources=resources, stack=stack):
        try:
...
def destroy() -> None:
    ...
    if is_user_approved(
        verb="destroy",
        resources=STACK_RESOURCE_MSG_DICT[stack] + STATE_RESOURCE_MSG,
        stack=stack,
    ):
        try:
            core.destroy()
